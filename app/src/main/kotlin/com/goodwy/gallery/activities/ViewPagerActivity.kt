...
class ViewPagerActivity : BaseViewerActivity(), ViewPager.OnPageChangeListener, ViewPagerFragment.FragmentListener {
        ...
        runOnUiThread {
            ...
            binding.mediumViewerToolbar.menu.apply {
                ...
                findItem(R.id.menu_rotate).setShowAsAction(
                    if (rotationDegrees != 0) MenuItem.SHOW_AS_ACTION_ALWAYS else MenuItem.SHOW_AS_ACTION_IF_ROOM
                )
                // OCR: só visível para imagens (não vídeo, não GIF)
                findItem(R.id.menu_extract_text)?.isVisible = currentMedium.isImage() && !currentMedium.isGIF()
            }
    ...
    private fun setupOptionsMenu() {
        ...
        binding.mediumViewerToolbar.setOnMenuItemClickListener { menuItem ->
            ...
            when (menuItem.itemId) {
                ...
                R.id.menu_extract_text -> extractTextFromCurrentImage()
    ...
    private fun initContinue() {
        ...
        if (intent.action == "com.android.camera.action.REVIEW" || intent.action == MediaStore.ACTION_REVIEW) {
    ...
    private fun animatePagerTransition(forward: Boolean) {
        ...
        animator.addListener(object : Animator.AnimatorListener {
            ...
            override fun onAnimationCancel(animation: Animator) { binding.viewPager.endFakeDrag() }
    ...
    private fun getChangeOrientationIcon(): Int {
        return if (mIsOrientationLocked) {
            if (requestedOrientation == SCREEN_ORIENTATION_PORTRAIT) R.drawable.ic_orientation_portrait_vector
            else R.drawable.ic_orientation_landscape_vector
        } else {
    ...
    private suspend fun isShowHiddenFlagNeeded(): Boolean = withContext(Dispatchers.IO) {
        val file = File(mPath)
        if (file.isHidden) return@withContext true
        var parent = file.parentFile ?: return@withContext false
        while (true) {
            if (parent.isHidden || parent.list()?.any { it.startsWith(NOMEDIA) } == true) return@withContext true
            if (parent.absolutePath == "/") break
    ...
    private fun initBottomActionButtons() {
        ...
        binding.bottomActions.bottomDelete.setOnClickListener { checkDeleteConfirmation() }
        ...
        binding.bottomActions.bottomRotate.setOnClickListener { rotateImage(90) }
        ...
        binding.bottomActions.bottomProperties.setOnClickListener { showProperties() }
        ...
        binding.bottomActions.bottomToggleFileVisibility.setOnClickListener {
            currentMedium?.apply {
                toggleFileVisibility(!isHidden()) { updateBottomActionIcons(currentMedium) }
            }
        }
        ...
        binding.bottomActions.bottomRename.setOnClickListener { checkMediaManagementAndRename() }
        ...
        binding.bottomActions.bottomCopy.setOnClickListener { checkMediaManagementAndCopy(true) }
        ...
        binding.bottomActions.bottomMove.setOnClickListener { moveFileTo() }
        ...
        binding.bottomActions.bottomResize.setOnClickListener { resizeImage() }

        // OCR no botão de extrair texto (se habilitado nas configurações da barra)
        binding.bottomActions.bottomExtractText.beVisibleIf(
            visibleBottomActions and BOTTOM_ACTION_EXTRACT_TEXT != 0 && currentMedium?.isImage() == true && currentMedium.isGIF() == false
        )
        binding.bottomActions.bottomExtractText.setOnLongClickListener { toast(R.string.extract_text); true }
        binding.bottomActions.bottomExtractText.setOnClickListener { extractTextFromCurrentImage() }
    ...
    private fun updateBottomActionIcons(medium: Medium?) {
        ...
        val favoriteIcon = if (medium.isFavorite) com.goodwy.commons.R.drawable.ic_star_vector else com.goodwy.commons.R.drawable.ic_star_outline_vector
        binding.bottomActions.bottomFavorite.setImageResource(favoriteIcon)

        val hideIcon = if (medium.isHidden()) com.goodwy.commons.R.drawable.ic_unhide_vector else R.drawable.ic_hide_vector
    ...
    private fun restoreFile() {
        restoreRecycleBinPath(getCurrentPath()) { refreshViewPager() }
    }

    private fun resizeImage() {
        launchResizeImageDialog(getCurrentPath())
    }
    ...
    private fun checkDeleteConfirmation() {
        if (getCurrentMedium() == null) return
        handleMediaManagementPrompt {
    ...
    private fun deleteConfirmed(skipRecycleBin: Boolean) {
        ...
        if (getIsPathDirectory(path) || !path.isMediaFile()) return
    ...
    private fun checkMediaManagementAndRename() {
        handleMediaManagementPrompt { renameFile() }
    }
    ...
    private fun renameFile() {
        ...
        RenameItemDialog(this, oldPath) {
            ...
            ensureBackgroundThread { updateDBMediaPath(oldPath, it) }
    ...
    private fun gotMedia(thumbnailItems: ArrayList<ThumbnailItem>, ignorePlayingVideos: Boolean = false, refetchViewPagerPosition: Boolean = false) {
        ...
        if (isDirEmpty(media) || media.hashCode() == mPrevHashcode) return

        val isPlaying = (getCurrentFragment() as? VideoFragment)?.mIsPlaying == true
        if (!ignorePlayingVideos && isPlaying && !isExternalIntent()) return
    ...
    private fun refreshUI(media: ArrayList<Medium>, refetchViewPagerPosition: Boolean) {
        ...
        if (refetchViewPagerPosition || mPos == -1) {
            ...
            if (mPos == -1) min(mPos, media.lastIndex)
        }
    ...
    override fun videoEnded(): Boolean {
        if (mIsSlideshowActive) swipeToNextMedium()
    ...
    private fun getCurrentMedium(): Medium? {
        return if (getCurrentMedia().isEmpty() || mPos == -1) null
        else getCurrentMedia()[min(mPos, getCurrentMedia().lastIndex)]
    }
    ...
    private fun isExternalIntent(): Boolean = !intent.getBooleanExtra(IS_FROM_GALLERY, false)
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────
    // OCR — Extrair texto de imagem via ML Kit
    // Acionado pelo item de menu "Extrair texto" ou botão da barra de ações.
    // ──────────────────────────────────────────────────────────────────────────────────────
